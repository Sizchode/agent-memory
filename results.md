...
最新目标：用户允许单次与多轮实验在多数条件下领先，不再要求所有单元格全胜。多轮主报告用 1、3、5 轮上限，已跑的 7 轮保留为补充；不删历史结果，不逐任务挑配置，不把任务平局计作严格获胜。该选择发生在开发期间，仍保留 test-as-dev 声明。报告入口移除旧的自动 6/6 达标与 5/6 里程碑标签，改为逐模型列胜、平、负及完整覆盖状态；原指标、原分数和缺失处理不变。
