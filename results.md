...
下一项只改事实使用方式：保留检索原文和我们原有的窗口，去掉原先全量事实附录，将同一批问题相关事实作为补充。不更改事实数、来源检索或任务参数，直接复用刚完成的选择结果；对 BM25/HippoRAG 2 同样保留各自原文并补充其已选事实。独立输出为该目录根下 `source_and_fact_context`。CPU 接口预检 `6540619` 已准备完六任务，依赖它的 GPU 预检为 `6540621`；尚未提交此修正版全量。
