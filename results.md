...
**单次 QA 的四 reader 目标已经达到。** 新的固定轨迹 IRCoT 三档曲线及独立的 Llama 每轮在线增强对照也已齐全，见下方。停止新增调参，进入方法回顾与消融设计阶段。这是反复使用测试集开发、单 seed 的本地比较，不是全领域 SOTA，也不是独立泛化证据；Qwen9 的单次 MH 仅领先一点，未证明统计显著。
...
Llama 全量作业 `6540627` 已 `COMPLETED 0:0`，用时 7:01:33。每档 3386 题，六任务完整，1/3/5 轮共 10158 条 QA；汇总重新核对逐题原生评分、题数及 reader 环境。该条件在每轮推理及最终 QA 使用旧完整事实附录与窗口，重新生成在线查询，不是主方法的十条相关事实配置。

| 对照 | 1 轮胜/平/负 | 3 轮胜/平/负 | 5 轮胜/平/负 |
|---|---|---|---|
| 相对 IRCoT + BM25 | 4 / 0 / 2 | 3 / 0 / 3 | 4 / 0 / 2 |
| 相对仅最终 QA 加旧完整附录 | 1 / 3 / 2 | 3 / 1 / 2 | 5 / 0 / 1 |

三轮详情如下，百分制：

| 任务 | BM25 | 图原文 | 仅最终 QA 完整附录 | 每轮及最终 QA 完整附录 | 主方法：最终 QA 相关事实 |
|---|---:|---:|---:|---:|---:|
| SH-Doc QA | 84 | 86 | 85 | 86 | 87 |
| MH-Doc QA | 57 | 59 | 54 | 61 | 64 |
| FactConsolidation-SH | 27 | 31 | 22 | 17 | 40 |
| FactConsolidation-MH | 1 | 4 | 0 | 0 | 9 |
| LoCoMo | 46.84 | 48.50 | 50.55 | 50.89 | 49.93 |
| 2WikiMultiHopQA | 54.18 | 50.19 | 40.98 | 40.87 | 52.46 |

在线增强相对仅最终 QA 增强有局部改善，但没有修复事实整合和 2Wiki 的退步，不升级为统一主方法，也不因此按 reader 切换配置。主方法仍是原始段落参与中间推理，最终回答使用原文、窗口及问题相关事实。不能把这项旧完整附录的负结果外推为所有在线上下文选择都无效。

| 轮数 | 最终 QA 题数 | 最终 QA 输入 tokens | 最终 QA 输出 tokens |
|---|---:|---:|---:|
| 1 | 3386 | 12458342 | 32766 |
| 3 | 3386 | 23425174 | 32872 |
| 5 | 3386 | 24958717 | 33157 |

这些仅为最终 QA 成本，不含中间推理、embedding 或 recognition；实际推理及 recognition 调用用量保存在各任务轨迹，不能把上表当成端到端成本。原生 8000-token 提示整理会删除示例、必要时从提示开头删行，因此并非全部增强文本无损进入每轮推理。没有为我们单独放宽该限制。

完整产物：`optimization_ircot_online_context_seed42_20260920/main/meta-llama_Llama-3.1-8B-Instruct`，包括 `comparison.json`、`complete.json`、各档输入、预测和轨迹。一次性 `run_online_context.py` / `.sbatch` 与 `code_6540627.zip` 逐字节核对、ZIP 完整性检查通过后移除，缓存及结果保留。至此本轮主实验及在线对照全部完成；系统性消融仍待回顾后启动。
