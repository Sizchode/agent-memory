...
全量 `6513732` 已提交，三项各覆盖全部 3386 题，设置固定，不对这 30 题调参。原题、切分、指标和五条出处上限保持不变，已有 recognition 缓存可复用，新请求记录实际 token 与耗时；均使用本地模型服务。实际输出在 `optimization_contextual_index_seed42_20260919`；预检只证明接口可运行，尚无全量改进或 SOTA 结论，主算法代码未改。
