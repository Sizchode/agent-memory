...
原排序对九项原生最佳为 2 胜 1 平 3 负，新增重排后为 3 胜 3 负，仍未达到 5/6。重排相对我们本次原排序为 1 胜 1 平 4 负；与同样重排的 BM25/HippoRAG 2 的逐任务最佳相比为 2 胜 2 平 2 负。不能将只赢重排后的部分 baseline 写成赢完整九项原生 baseline。该组件不升级为统一 one-shot 主方法，不据 MH 单项提升扩展四 reader。IRCoT 重排是另一个尚在完成中的比较，不能用此结果提前替代其结论。
