...
完整三元组清单下的八配置、两个模型、六任务 QA 已完成，共 96 个结果格；尚未切换默认。
相邻原文范围和每路候选数量的敏感性检查正在进行，完成后再冻结简化版。
...
对照为原版 A-MEM 与接入我们图的 A-MEM，保留记忆写入与演化机制；计划在最终检索阶段接入图，不能让完整离线图向较早的记忆更新泄漏未来资料。
六任务范围不变，接口不兼容时明确报告；当前尚无接入预检或正式结果，不能把普通多轮 RAG 计为该实验。
用户希望 2026-09-18 中午看到结果，按 America/New_York 的 12:00 理解（16:00 UTC）。这作为 Gemma/Llama 全量 QA 的执行目标，不作为全部作业必然完成的承诺。
A-MEM 尚需实际预检、记忆构建及端到端评测，暂无可信的完成时间估计；不能将小样本检查或部分任务标成全量结果。
...
当前证据支持把候选过滤作为下一项可删除模块；其他单项改动均未同时满足两个模型至少 5/6。
代码尚未删除候选过滤，默认仍是 9B 5/6、4B 6/6。完整清单版本会让 4B FCMH 从默认的 8 降到 4，低于最佳 baseline 的 5，不能称完全无损。
...
QA 为 6479441（9B）、6479442（4B），分别等待上一个对应模型作业结束，仍最多同时两张 H100；4B 已完成，9B 正在最后一组。
产物为 `optimization_window_sensitivity_seed42_20260917`，结果尚未收齐。
每路候选数检查为 5、10、20，最终仍返回 5 个结果；构建 6479858 已成功，检索 6479859 正在运行。
要求候选数 5 精确重现全部参考 QA 输入；复用原查询向量和识别缓存，不改变原模型、划分或指标。
产物为 `optimization_rank_sensitivity_seed42_20260917`，QA 为 6479940（9B）、6479941（4B），分别依赖检索成功及对应窗口 QA 完成，当前等待中。
这两项都是现有参数检查，不新增选参规则。运行入口已允许将最终方法路径传给 Gemma/Llama 准备步骤，但尚未启动新增 reader 主实验。
