...
Gemma/Llama 的九 baseline 加最终方法主实验继续，不因这次调整重跑或改参。IRCoT 官方版本为 `3c1820f698eea5eeddb4fba3c56b64c961e063e4`，准备作业 `6483641` 仅检查依赖、官方提示词、六任务原始题目与已验证图；尚未运行正式多步检索。计划复用官方控制流程，比较 BM25、HippoRAG 原图、我们的图，六任务共用同一配置，保留原 loader 与 QA 评分。
...
接口测试直接运行官方参与模块及控制器，用合成语料、固定响应检验答案结束、十轮上限和空输出三种情形，分别产生 2/10/1 次检索；不调用真实 LLM，不计作 benchmark 成绩。适配器仅替换检索与模型传输接口，使用来源顺序编号作为检索接口的标题标签、保留原文文本，不用相似度猜来源；正式图后端及本地 reader 的联合预检尚待完成。
真实联合预检已提交：Gemma `6483798` 依赖其单次 QA `6482985`，Llama `6483800` 依赖 `6482984`，各覆盖六任务第一题与 BM25/原图/新图三组，共 18 个配置/问题组合。事实识别服务为 `6483797`，仍最多三张 H100；未提交正式全量多步检索，须先检查真实预检。
两个初次真实预检均在创建 `reader.json` 时失败：模型目录未创建，尚未进入检索。已补充目录创建并将元数据写入纳入 reader 关闭保护，未改实验设置；失败日志保留。替代预检为 Gemma `6484052`、Llama `6484053`，单次 QA 已完成，因此无需继续等待其依赖。
