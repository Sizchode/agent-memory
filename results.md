...
最新状态：同版本源码构建 `6523923` 成功（15 秒），NumPy 2.4.6 下的上游复现例和连接小图均通过索引检查。检索预检 `6523947` 完成 30 题、15 组，全部返回 2–5 条原文；Gemma `6523988` 完成五配置共 150 条原生 QA 预测。25/30 题的 PCST 原文集合与 dense 前三条相同，因此补上 dense 前三条的消融，预检 `6524109` / `6524110` 已提交。全量检索 `6524127` 和 Gemma QA `6524128` 依次等待预检、完整检索成功；尚无全量 PCST QA 结论。这些预检只用于端到端检查，不据此选择参数。
