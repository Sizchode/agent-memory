...
固定 cap=3、固定已有轨迹的四项 QA 对照已提交 Gemma 试跑 `6511321`：原顺序全部材料、重排全部材料、原顺序前 6 条、重排前 6 条。官方 BM25 与图后端都测，均沿用现有最终 QA；使用本地 Qwen3-Reranker-0.6B，通用官方指令，不训练权重。原顺序对照在本次同一硬件重新作答，而不是直接把旧硬件成绩当对照。最近状态为 `PENDING / QOSMaxGRESPerUser`，未产生 QA 结论；试跑不是全量，且这些最终材料处理不是新的在线 IRCoT 轨迹。脚本与产物位于 scratch，协议与实现细节见 `research_plan.md`。
...
Gemma 六任务全量固定轨迹对照已提交为 `6511907`，四处理与两后端均完整保留，输出在该目录 `main`；尚无全量结论。冻结事实连接候选的八项 QA 预检 `6511851` 也已启动，使用相同 30 题、同一 Gemma reader、原生提示与指标，对照 BM25 与相同重排器，不与旧图主表合并。输出为 `optimization_fact_evidence_pilot_seed42_20260919`，运行代码与候选均归档在产物中。
...
下一项预检 `6512301` 固定这些候选，比较 BM25、两种图评分及各自与 BM25 的标准 RRF 融合，共五项，常数仍为已有的 60、输出仍为六条，不按任务调参。检索阶段不增加模型调用。保存的 IRCoT 排名不带原始 BM25 分数；融合接口现允许没有分数的有效排名，不伪造分数，明确非正 BM25 分数仍排除。旧在线 BM25 的有分数路径不变。此处融合是已有方法，不作为 novelty；是否能补足图候选缺失由同次原生 QA 检验。

该接口的 CPU 检查 `6512339` 已成功，41 项通过、3 项旧产物依赖测试跳过；测试脚本及代码快照已归档，活动 launcher 删除。全量原文对照恢复 `6512216` 已通过所有已存文件一致性检查，完成六任务证据准备并进入 QA；本次纯 forward 记录为 168.12 秒，35305 对、7578034 输入 token，保留之前中断作业的独立用量。尚未据此报告全量 QA 优劣。

关系评分准备 `6512008` 已完成，30 题旧候选全部重放一致，新关系评分仍为 23/6/1（匹配/无匹配/解析无效）；不能从此计数判断 QA 收益。其 QA `6512188` 尚待完成。全量原文重排原作业 `6511907` 因共享盘重复缓存查询过慢而正常中断，用量已保存；恢复为 `6512216`，只优化内存缓存、保留已提交分数及相同实验协议。原作业 15 分 23 秒中，已记录纯模型 forward 为 100.18 秒；不把未完成运行写成成功或把这两种时间混用。

后续固定模式的关系评分对照正在运行：CPU `6512008` 用相同实体约束和搜索设置比较整条三元组评分与已有关系距离算子；GPU QA `6512188` 依赖它成功，再于同次 Gemma 运行中比较两种评分和 BM25。不是重新生成解析，不新增抽取，也不是完整 SimGRAG 复现。单元检查 40 项通过、3 项跳过，尚无这项对照的 QA 分数。全量原文重排作业 `6511907` 仍独立运行，两项实验不拼接成绩。
...
`6512216` 已完成以下五任务的全部八项处理。各格为同次 Gemma/L40S 的 BM25 / 旧图；固定已有 cap=3 轨迹，不改变中间推理。四项 MAB 各 100 题，指标为 substring EM；LoCoMo 全部 1986 题，指标为原生 F1。均为百分制，不跨任务求平均。2Wiki 尚未全项完成，不报告六任务胜出数。
...
另已启动六任务全部 3386 题的固定查询解析准备 `6512502`，复用 30 题缓存，失败不剔除。其产物只是检索组件输入，不是 QA 结果；查询模型成本完整计入，不把预计算当作免费离线索引。具体协议见 `research_plan.md`。
