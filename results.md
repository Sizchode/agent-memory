...
## 当前安排：保留旧完整方法，优先改善 Llama

### 新进展：保留原文的事实补充配置在 Llama 达到原生九 baseline 的 5/6

`6540635` 已生成完整成功标记，三种方法各六任务全量，共 10158 条 QA，并逐题重评分。图、来源检索、窗口、OpenIE 和向量不变；保留完整原文，只将原先全量事实附录换成问题相关的十条事实。固定十条和相似度选择沿用先前 KAPING 组件检验，不按任务调参，也不是完整 KAPING 复现。Llama 结果如下，百分制：

| 任务 | 旧完整方法 | 原文加相关事实 | 九项原生最佳 |
|---|---:|---:|---:|
| SH-Doc QA | 87 | 87 | 89 |
| MH-Doc QA | 56 | 60 | 56 |
| FactConsolidation-SH | 32 | 39 | 34 |
| FactConsolidation-MH | 0 | 5 | 2 |
| LoCoMo | 49.57 | 49.46 | 48.00 |
| 2WikiMultiHopQA | 43.94 | 50.63 | 42.07 |
...
下一项 IRCoT 使用同一事实补充操作，仍固定既有轨迹、原文条数和顺序，只改最终 QA 上下文；按原问题选择十条事实，不使用答案或新增子问题。CPU 预检 `6541026` 已成功，四 reader 的 GPU 接口预检 Llama/Qwen4/Qwen9/Gemma 为 `6541029` / `6541031` / `6541036` / `6541037`，各为三档轮数、六任务、90 条 QA。其中 Llama、Qwen4 已全部通过；Llama 预检终态 `COMPLETED 0:0`，用时 1 分 48 秒。按同一设置提交二者全量 `6541203` / `6541210`，各新增 10158 条 QA，均依赖 CPU 全量上下文准备 `6541194` 成功；其余 reader 仍待各自预检通过。对照复用已完成且 reader 环境一致的 BM25 原文、图原文及全量附录条件，实际 token 长度先检查，超长报错不删题。目录 `optimization_ircot_fact_context_seed42_20260920`；不预报收益。
