...
后三个单次 reader 的预检最初受 `gpu-he` 的 `QOSMaxGRESPerUser` 限制，未运行时改到用户账户允许的普通 `gpu` / `norm-gpu`，全量同样通过 sbatch 命令行覆盖分区与 QOS；Gemma 保持 `gpu-he`。所有作业保持 L40S、单 CPU、同一模型版本和原生推理配置，不因分区调整更改评测条件。
