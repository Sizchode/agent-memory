...
另已启动六任务全部 3386 题的固定查询解析准备 `6512502`，复用 30 题缓存，失败不剔除。四项 MAB 和 LoCoMo 已完成；LoCoMo 为 1759 合法、114 空模式、113 非法，2Wiki 继续。其产物只是检索组件输入，不是 QA 结果；查询模型成本完整计入，不把预计算当作免费离线索引。

全量单次检索 `6512715` 依赖解析成功，Gemma QA `6512717` 依赖检索成功。冻结两项事实评分，恢复原单次 QA 的五条出处上限，与原 BM25、HippoRAG 2 同次重新 QA，覆盖六任务全部 3386 题。当前是已提交的候选验证，尚无成绩；不把前面六条预算的预检表当作这批结果。具体协议见 `research_plan.md`。
