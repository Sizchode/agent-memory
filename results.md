...
`6513407` 已启动：Gemma、L40S、seed=42，先做 30 题接口预检，再对全部 3386 题同时重新评测“原文”和“原文加已选三元组”。固定上述关系检索结果，保留全部空匹配，不增加后备检索、不改解析或评分。三元组只附在其原始出处后，不声称是真值。该项检验 reader 输入，既不是新图，也不是最终主实验。

接口测试 49 项中 46 项通过、3 项旧产物依赖项跳过。实际生成输入与 canonical 检索记录分开保存，QA 使用前者，原生段落召回使用后者；新测试验证段落原文未改变、LoCoMo 选项随机顺序与生成设置一致，且附录不会令段落召回误变为零。附录增加的输入 token 单独计入，不能把这项称为等预算结果。当前尚无全量 QA 结论，产物独立位于 `optimization_fact_context_seed42_20260919`。
