...
Qwen 扩展准备：`run_ircot.py` 已支持 4B/9B，配置检查 `6484247` 成功，四模型本地 config/tokenizer 均可读取，模型版本与上下文限制写入 `reader_preflight.json`。这是 CPU 检查，未测试 Qwen 的实际推理或完成其 benchmark。已有 Gemma/Llama 汇总仍只要求原两个 reader；四模型汇总须显式指定四个模型并等待各自全量 QA。真实联合预检已提交：4B `6484343` 依赖 Gemma 旧版对照 `6484256`，9B `6484344` 依赖 Llama 旧版对照 `6484258`，各一张 H100、单 CPU，六任务各一题乘三种后端。仍不超过三张 H100 并发；启动时检查事实识别服务健康，失败则报告而非降级。Qwen 全量作业尚未提交，须先确认真实预检通过。
