...
相邻原文范围敏感性检查已完成，共 60 个完整任务结果，覆盖 33860 个配置/模型/问题组合；每路候选数量的 QA 已完成 9B，4B 仍在运行。
...
候选数 10、20 已由完成的 MAB 结果排除，选用完整候选索引、每路候选数 5、RRF 常数 60、相邻原文范围 3，不再扩展配置搜索；剩余全量结果仍继续运行。
...
候选数量消融 `6479940`（9B）已成功，`6479941`（4B）仍在运行。汇总 `6482770` 依赖两个作业成功；Gemma/Llama 主实验仍须等待完整消融汇总。
...
9B QA 已成功结束，4B 仍在运行。候选数 10 的已完成 MAB 结果为 9B 的 90/57/62/7，4B 的 89/56/60/5；任务顺序为 SH、MH、FCSH、FCMH。
...
候选数 20 的 MAB 四项已完成：9B 为 91/58/61/10，4B 为 90/52/56/6。两模型各有两项未严格胜出，均最多 4/6，同样不能采用；完整六项表尚待收齐。候选数因此固定为 5，后续 reader 不再参与选参。
