...
验证作业 `6482036` 已通过 25 项构图测试及 15 组独立重建；3386 题普通接口检索核对正在运行，尚不能标为全量验证通过。
四个 MAB 任务已分别核对 100/100 题一致；LoCoMo、2Wiki 的完整核对结果尚未落盘。
...
相邻原文范围敏感性检查已完成，共 60 个完整任务结果，覆盖 33860 个配置/模型/问题组合；每路候选数量的 QA 仍在排队，完成后再冻结配置。
...
三组 A-MEM 对照均使用该适配，明确区别于未经修改的官方实现。重新提交真实预检查 `6482875`；这次暂不提交新的完整构建，待六任务预检查实际通过后再继续。
...
候选数量消融 `6479940`（9B）、`6479941`（4B）均已开始。汇总 `6482770` 依赖两个作业成功；Gemma/Llama 主实验仍须等待方法冻结。
...
两个 QA 作业现均在运行。候选数 10 的已完成 MAB 结果为 9B 的 90/57/62/7，4B 的 89/56/60/5；任务顺序为 SH、MH、FCSH、FCMH。
9B 已在 MH、FCSH 两项未超过最佳 baseline，因此该配置最多 4/6，不满足选用门槛。六任务仍完整评测，不以部分结果替代完整消融表；候选数 20 尚无完整结论。
