...
代码提交 `8624980` 将新图加保留事实索引设为默认，保留原索引四格与旧图同索引的明确入口。
本轮主线为 **Consolidate Before You Propagate**：同一保留事实支持派生传播图和识别候选。
“减少历史干扰”是机制解释假设，不是所有分差的已证实原因；来源顺序仍是经验政策，
已有投影性质不作为新定理。英文任务覆盖及 test-as-dev 开发协议已写入 research_novelty.md。
docs/research_plan.md 的三 reader 计划标为历史，README 的复现示例改为已跟踪的 Python 入口。

从 `8624980` 导出独立干净目录后，batch 作业 `6390831` 完成，0:0（58 秒）：
29 项测试和主入口导入通过；不依赖未跟踪的旧执行脚本。六项任务全部核对原 case 顺序，
六组完整配置的实际检索输入、五中心及逐题原生成设置一致；不是重新调用模型。
调用原评分器重新评分九项 baseline 共 108 个任务/reader 单元、60948 条预测，
六组配置共 72 个单元、40632 条预测，核对 summary、完整覆盖、QA usage 与严格胜出数。
统一主配置仍为 9B/4B 双 6/6；该检查不产生可供择优的新 QA 结果。

| 完成项 | 权威证据与范围 |
|---|---|
| 原始/新构图分别有无 refinement | 同 H100 四格完整结果；报告 6378444，最终干净提交再次核对四格全部预测 |
| 候选索引与图构造分离对照 | 新图新索引 6387725、旧 refinement 图同索引 6389297；最终再次核对全部预测与输入 |
| 做厚做薄 | 显式事实节点、自环投影、全部支持、raw relation、删除额外直连等尝试及完整取舍保留；退役实现已归档，当前无独立废弃入口 |
| 默认配置可实际加载 | 独立重建 6389786，15 组图/14362 来源/167074 候选核验 6389835；六任务 3386 题普通接口验证 6389842 至 6389848 |
| 代码与协议回归 | 默认入口 6390450 的 29 项测试；干净提交 6390831 的测试及全量评分/输入审计；本轮不改 loader、指标或 QA |
| 结果、缓存与文档保存 | outputs 中完整产物及 implementation 归档保留；legacy_notes.zip 再次通过完整性检查；当前方法与历史计划分开 |

这是当前授权的离线构图与索引优化收敛，不是全局最优或全领域 SOTA 证明。
未实现的在线增量维护、PL 正确性、额外种子实验不计为已完成成果，也不作为本轮方法的主张。
新图 SH 和 9B FCMH 对旧图同索引有回退；不同上下文长度和采样分差的归因限制继续保留。
提交范围仅为本轮算法入口、既有测试与研究文档；其他已暂存子模块和 baseline 改动原样保留。
