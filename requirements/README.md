...
create one environment per method and one independent service environment:

```bash
uv venv .venv/lightmem --python 3.11
uv pip install --python .venv/lightmem/bin/python -r requirements/lightmem.in

uv venv .venv/magma --python 3.11
uv pip install --python .venv/magma/bin/python -r requirements/magma.in

uv venv .venv/hipporag --python 3.11
uv pip install --python .venv/hipporag/bin/python -r requirements/hipporag.in

uv venv .venv/mem0 --python 3.11
uv pip install --python .venv/mem0/bin/python -r requirements/mem0.in

uv venv .venv/inference --python 3.11
uv pip install --python .venv/inference/bin/python -r requirements/inference.in
```
