...
后续讨论已将目标 reader 收敛为 Qwen3.5-4B、9B，2B 仅保留历史结果。
当前采用统一的来源支持图索引：新无自环构图加保留事实候选，在六项完整任务上
对九项既定本地 baseline 均为双 6/6 严格胜出。旧 refinement 图配相同候选索引
为 9B 4/6、4B 6/6；新图并非逐任务支配，完整结果与开发集口径均保留。
原候选索引下的新旧构图/refinement 四格及后续删减已完成，获胜配置已独立重建，
全部 3386 题普通检索接口验证通过。该结果不是全领域 SOTA 或独立测试泛化。

当前实现与复现入口见 [optimization/README.md](../optimization/README.md)，
方法、相关工作与英文实验范围见 [research_novelty.md](../optimization/research_novelty.md)，
全部尝试及取舍见 [record.md](../optimization/record.md)。
以下保留早期计划，不再作为当前 reader 数量、阶段顺序或默认配置的要求。

## 历史计划：分布内图优化，三个模型各达到 5/6 最优
