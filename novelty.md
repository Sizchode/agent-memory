...
完整三元组清单版本的八配置消融现已全部完成，尚未切换默认实现。在该版本上继续做单项改动，结果如下：
...
| 删除关键词搜索与排名合并 | 3/6 | 4/6 |
| 不添加相邻原文 | 5/6 | 4/6 |
| 不附三元组列表 | 5/6 | 2/6 |
| 恢复相似实体连接 | 4/6 | 3/6 |
| 换回 HippoRAG 原图 | 3/6 | 2/6 |

所以，候选过滤是目前有证据支持继续删除的一步；其他单项改动没有同时满足两个模型至少 5/6。候选过滤代码还没删除，相关参数检查完成后再确定最终版本。恢复完整清单也并非无损：4B 的 FactConsolidation-MH 从默认配置的 8 降到 4。

### 新图本身有用吗？已有支持，但要说清对照范围

已经完成两组这样的对照。上表采用当前筛选后的事实出处，固定完整三元组清单及材料整理规则，只换回原图，成绩从 6/6、5/6 降为 3/6、2/6。但并非每项都下降，例如两个模型的 LoCoMo 使用原图反而更高。

另一组实验保留全部事实出处，固定完整三元组清单、排名融合及参考材料整理规则，只替换图。原图为 9B 3/6、4B 2/6，新图为两个模型各 5/6。
